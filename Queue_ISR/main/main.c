...
void IRAM_ATTR button_isr_handler(void *arg)
{
    ...
    xQueueSendFromISR(queue1, &txbuff, ISR);
}
...
void app_main()
{
    ...
    gpio_isr_handler_add(CONFIG_BUTTON_PIN, button_isr_handler, ISR);

    xTaskCreate(task1, "task1", 4096, NULL, 10, task1Handle);
    xTaskCreatePinnedToCore(task2, "task2", 4096, NULL, 10, task2Handle, 1);
}